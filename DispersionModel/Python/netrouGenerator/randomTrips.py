...
def get_options(args=None):
    random.seed(1) # add a random seed to generate route file with same source and destination (XZ)
        ...
        try:
            ...
            if options.pedestrians:
                ...
                if options.persontrips:
                    ...
                    range(8), weights=(80, 80, 80, 10, 10, 10, 80, 80))
                    index = 5#index[0]
                    busStop = ["bs_0", "bs_1", "bs_2", "bs_3", "bs_4", "bs_5", "bs_6", "bs_7"]
                    busStopEdge = ["474688712#0", "626284345#0", "626283153#0", "475159102#1", "-474689217", "626283217#0", "475159103#0", "474688711"]
                    fouttrips.write(
#                        '<walk from="%s" to="%s" arrivalPos="random"/>\n' % (
#                            source_edge.getID(), source_edge.getID()) + 
                         

                        '<walk from="%s" busStop="%s" arrivalPos="random"/>\n' % (
                            source_edge.getID(), busStop[index]) + 
                        '<ride from="%s" to="%s" lines="%s"%s/>\n' % (
                            busStopEdge[index], sink_edge.getID(), options.personrides, otherattrs))
                    #print('        <walk%s%s%s/>\n' % (attrFrom, attrTo, otherattrs))
                    #fouttrips.write(
                    #    '        <walk%s%s%s/>\n' % (attrFrom, attrTo, otherattrs))
                else:
