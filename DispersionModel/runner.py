...
def expanding2D(conc, gridSize, expanding, timeStep, windX, windY):
    ...
    L_e_y_plus = ((timeStep * expanding) / 2) + windY * timeStep
    L_e_x_plus = ((timeStep * expanding) / 2) + windX * timeStep
    L_e_y_minus = ((timeStep * expanding) / 2) - windY * timeStep
    L_e_x_minus = ((timeStep * expanding) / 2) - windX * timeStep
...
def dispersion(timeGap, concLast, X, Y, Z, gridSize, windEnvX, windEnvY, vehicleInfo, emissionRate, expanding, fading, decay, fadingThreshold, bound):
    ...
    for iz in range(len(Z)):
        ...
        if row.size > 0:
            for j in range(row.size):      
                ...
                if (row[j] - 1 >= 0):
                    concCurr[row[j] - 1, col[j]] = concCurr[row[j] - 1, col[j]] + neighbors[1]
                ...
                if (row[j] + 1 < int (X / gridSize)):
                    concCurr[row[j] + 1, col[j]] += neighbors[7]
                    if (col[j] + 1 < int (Y / gridSize)):
...
def dispersion2(timeGap, concLast, X, Y, Z, gridSize, vehicleInfo, emissionRate, bound, windEnvX, windEnvY, Kx, Ky, Kz):
    ...
    for veh_id in vehicleInfo:
        ...
        if emissionPosX >= 0 and emissionPosX < concLast.shape[0] and emissionPosY >= 0 and emissionPosY < concLast.shape[1]:
            ...
            concLast[emissionPosX, emissionPosY, 0] += 0#er[6] * timeGap / (gridSize**2*(Z[-1]- Z[-2])) # change concCurr to concLast
        else:
...
def Advect(windX, windY, timeStep, concLast, X, Y, gridSize):
    ...
    for ix in range(int (X / gridSize) + 1):
        for iy in range(int (Y / gridSize) + 1):
            ...
            i = min(max(0, int (Xn / gridSize) ), X)
            Yn = iy * gridSize - windY[ix, iy] * timeStep
            j = min(max(0, int (Yn / gridSize) ), Y)
...
def run(helpers, fig, bound, dispersionModel):
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if dispersionModel == "2D-grid":
            concCurr = dispersion(timeGap, concLast, X, Y, Z, gridSize, windEnvX, windEnvY, vehicleInfo, emissionRate, expanding, fading, decay, fadingThreshold, bound)
        if dispersionModel == "3D-grid":
            concCurr = dispersion2(timeGap, concLast, X, Y, Z, gridSize, vehicleInfo, emissionRate, bound, windEnvX, windEnvY, gridSize*2, gridSize*2, Kz)
        concLast = concCurr
        ...
        for ped_id in personInfo:
            ...
            if traci.person.getStage(ped_id, 0).description is not "driving":
                # Computes inhale
        ...
        if traci.simulation.getTime() % 1200 == 0:
            with open("logs2\Log_concentration.npy","wb") as file:
                np.save(file, Log_concentration)
            with open("logs2\Log_pedestrain.json","w") as file:
                json.dump(Log_pedestrain, file)
            with open("logs2\Log_vehicle.json","w") as file:
        ...
        levels = np.arange(0, max(np.max(concCurr[:, :, 2]), 0.000001), max(np.max(concCurr[:, :, 2]), 0.000001) / 100)
        cs = plt.contourf(x, y, concCurr[:, :, 2].T, levels, cmap=plt.get_cmap('nipy_spectral_r'))
